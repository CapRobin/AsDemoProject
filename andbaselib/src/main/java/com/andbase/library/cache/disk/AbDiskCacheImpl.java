...
public class AbDiskCacheImpl implements AbDiskCache {
    ...
    /**  单例. */
    private static AbDiskCacheImpl diskCache;
     ...
     * 构造.
     */
    public AbDiskCacheImpl(Context context) {
     ...
     * @return
     */
    public static AbDiskCacheImpl getInstance(Context context) {
        if(diskCache == null){
            diskCache = new AbDiskCacheImpl(context);
        }
        return diskCache;
    }
    ...
    public synchronized void initialize() {
        if (!cacheDir.exists()) {
            if (!cacheDir.mkdirs()) {
                AbLogUtil.e(AbDiskCacheImpl.class,"缓存目录创建失败，"+cacheDir.getAbsolutePath());
            }
            return;
        }
    ...
    public synchronized void clear() {
        ...
        mEntries.clear();
        mTotalSize = 0;
        AbLogUtil.d(AbDiskCacheImpl.class,"Cache cleared.");
    }
     ...
     */
    @Override
    public synchronized AbDiskCacheEntry get(String key) {
        ...
        AbLogUtil.d(AbDiskCacheImpl.class, "想要从缓存中获取文件"+file.getAbsolutePath());
     ...
     */
    @Override
    public synchronized void put(String key, AbDiskCacheEntry entry) {
        ...
        if (!deleted) {
            AbLogUtil.d(AbDiskCacheImpl.class,"缓存文件删除失败"+file.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            AbLogUtil.d(AbDiskCacheImpl.class,"缓存文件删除失败");
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        while (iterator.hasNext()) {
            ...
            if (deleted) {
               ...
               AbLogUtil.d(AbDiskCacheImpl.class,"Could not delete cache entry for key=%s, filename=%s",
        ...
        if (AbLogUtil.D) {
        	AbLogUtil.d(AbDiskCacheImpl.class,"pruned %d files, %d bytes, %d ms",
    ...
    static class CacheHeader {
         ...
         */
        public CacheHeader(String key, AbDiskCacheEntry entry) {
            this.key = key;
         ...
         * @return the entry
         */
        public AbDiskCacheEntry toCacheEntry(byte[] data) {
            AbDiskCacheEntry e = new AbDiskCacheEntry();
            e.data = data;
            e.etag = etag;
        ...
        public boolean writeHeader(OutputStream os) {
            try {
                ...
                AbLogUtil.d(AbDiskCacheImpl.class,"%s", e.toString());
                return false;
            }
        }
     ...
     */
    public AbHttpCacheResponse getCacheResponse(String url,String sessionId){
        URLConnection con = null;
        InputStream is = null;
        AbHttpCacheResponse response = null;
        try {
            ...
            response = new AbHttpCacheResponse(data, mapHeaders);
     ...
     * @return
     */
    public AbDiskCacheEntry parseCacheHeaders(AbHttpCacheResponse response,long cacheTimeMillis) {
        ...
        AbDiskCacheEntry entry = new AbDiskCacheEntry();
